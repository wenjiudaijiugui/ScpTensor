...
class SimilarityMetrics:
    ...
    def check_thresholds(cls, metrics: Dict[str, float]) -> bool:
        ...
        if max_rel > cls.THRESHOLD_MAX_REL_ERR:
            return False
    ...
    def compare_results(
        ...
        # Check for NaN/Inf
        scanpy_has_nan = np.any(np.isnan(scanpy_result))
        scanpy_has_inf = np.any(np.isinf(scanpy_result))
        scptensor_has_nan = np.any(np.isnan(scptensor_result))
        scptensor_has_inf = np.any(np.isinf(scptensor_result))

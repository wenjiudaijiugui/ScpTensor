...
from scptensor.viz import embedding, qc_completeness, qc_matrix_spy
...
def run_pipeline():
    ...
    data_path = os.path.join(
        script_dir, "data/PXD061065/20250204_112949_gbm_sc_full_fasta_19.4_Report.tsv"
    )
    ...
    genes_col = "PG.Genes"
    data = data.select([genes_col, *quantity_columns])
    ...
    X[is_nan] = (
        0  # Fill with 0 or keep NaN? ScpMatrix usually stores values. Let's keep 0 for sparse compatibility or specific value.
    )
    ...
    obs = pl.DataFrame({"sample_id": sample_ids, "group": groups, "batch": batches})

    # Create var (Features metadata)
    var = pl.DataFrame({"protein_id": protein_ids, "_index": protein_ids})
    ...
    container.assays["protein"].layers["imputed"] = ScpMatrix(
        X=imputed_layer.X, M=np.zeros_like(imputed_layer.X, dtype=np.int8)
    )

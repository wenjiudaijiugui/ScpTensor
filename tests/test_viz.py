...
from scptensor.viz.base import scatter, heatmap, violin
...
def viz_container():
    ...
    obs = pl.DataFrame({
        ...
        "batch": ["batch1", "batch1", "batch1", "batch2", "batch2",
                  "batch2", "batch1", "batch1", "batch2", "batch2"],
        "group": ["A", "A", "B", "B", "A", "B", "A", "B", "A", "B"],
        "n_detected": [100, 120, 90, 110, 105, 95, 130, 85, 115, 125],
        "treatment": ["control", "control", "treated", "treated", "control",
    ...
    umap_assay = Assay(
        var=pl.DataFrame({"_index": ["dim1", "dim2"]}),
        layers={"X": ScpMatrix(X=X_umap)}
    )

    # Add PCA coordinates
    X_pca = np.random.rand(10, 3)  # 3D for testing dimension validation
    pca_assay = Assay(
        var=pl.DataFrame({"_index": ["PC1", "PC2", "PC3"]}),
        layers={"X": ScpMatrix(X=X_pca)}
    )

    return ScpContainer(obs=obs, assays={
...
class TestEmbedding:
    ...
    def test_embedding_basis_low_dimension(self, viz_container):
        ...
        assay_1d = Assay(
            var=pl.DataFrame({"_index": ["dim1"]}),
            layers={"X": ScpMatrix(X=X_1d)}
        )
...
class TestVisualizationEdgeCases:
    ...
    def test_volcano_with_nan_values(self, volcano_container):
        ...
        volcano_container.assays["protein"].var = volcano_container.assays["protein"].var.with_columns(

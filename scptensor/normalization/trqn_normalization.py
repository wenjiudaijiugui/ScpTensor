...
def _rank_invariance_frequency(
    ...
    """Compatibility wrapper over the shared internal TRQN helper."""
    return rank_invariance_frequency(feature_sample, qn_feature_sample=qn_feature_sample)
...
def norm_trqn(
    ...
    x_sample_feature = np.asarray(ensure_dense_matrix(input_layer.X), dtype=float)
    feature_sample = x_sample_feature.T
    n_features = feature_sample.shape[0]

    # Baseline quantile normalization for all features.
    qn_sample_feature = quantile_normalize_dense_rows(x_sample_feature)
    ...
    if selected_idx.size > 0:
        ...
        balanced_norm = quantile_normalize_dense_rows(subset.T).T

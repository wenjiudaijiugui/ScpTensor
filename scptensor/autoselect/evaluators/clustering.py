...
class ClusteringEvaluator(BaseEvaluator):
    ...
    def _result_output_kind(self) -> Literal["obs"]:
        """Clustering writes labels to ``obs`` rather than assay layers."""
        return "obs"
    ...
    def _prepare_evaluation_context(
        ...
        """Track the matrix actually used for clustering metrics."""
        self._metric_assay_name = assay_name
        self._metric_source_layer = source_layer

    def _clear_evaluation_context(self) -> None:
        """Reset to explicit clustering defaults after one evaluation."""
        self._metric_assay_name = "pca"
        self._metric_source_layer = "X"

    def _get_result_name(self, method_name: str, source_layer: str) -> str:
        """Return the obs key generated by each clustering candidate."""
        del source_layer
        if method_name == "kmeans":
            return f"kmeans_k{self._n_clusters}"
        if method_name == "leiden":
            return f"leiden_r{self._resolution}"

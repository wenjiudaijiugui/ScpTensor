...
IMPORT_REPLACEMENTS_LIST = [
    ...
    ("from scptensor.normalization.log import log_normalize",
     "from scptensor.normalization.log import norm_log as log_normalize"),  # Keep alias for test consistency
...
def update_file(file_path: Path) -> bool:
    ...
    if file_path.name == "test_normalization.py":
        ...
        if "from scptensor.normalization" in content:
            ...
            new_lines = []
            for line in lines:
                if "from scptensor.normalization." in line and "import" in line and " as " not in line:
    ...
    if "test_workflows.py" in str(file_path):
        ...
        content = content.replace('assert last_log.action == "log_normalize"', 'assert last_log.action == "norm_log"')
        content = content.replace('assert last_log.action == "knn"', 'assert last_log.action == "impute_knn"')
        content = content.replace('assert last_log.action == "integration_combat"', 'assert last_log.action == "integrate_combat"')
        content = content.replace('assert last_log.action == "basic_qc"', 'assert last_log.action == "qc_basic"')
    ...
    if file_path.name == "test_impute.py":
        # Update action name assertions
        content = content.replace('assert result.history[-1].action == "impute_knn"', 'assert result.history[-1].action == "impute_knn"')
        content = content.replace('assert result.history[-1].action == "impute_ppca"', 'assert result.history[-1].action == "impute_ppca"')
        content = content.replace('assert result.history[-1].action == "impute_svd"', 'assert result.history[-1].action == "impute_svd"')

...
| Job | Purpose |
|-----|---------|
| `benchmark` | Run benchmark entrypoint CLI smoke checks (`--help`) |
| `regression-test` | Run synthetic normalization benchmark smoke (`literature_matched`, 6 scenarios) |

---

### 5. Docs CI (`docs-ci.yml`)

**Triggers:**
- Push to `main` or `develop` when docs contracts change
- Pull requests to `main` or `develop` when docs contracts change
- Manual workflow dispatch
...
Use the workflow pages below as stable badge-generation entrypoints in GitHub UI:

- `CI`: <https://github.com/wenjiudaijiugui/ScpTensor/actions/workflows/ci.yml>
- `CD`: <https://github.com/wenjiudaijiugui/ScpTensor/actions/workflows/cd.yml>
- `Docs CI`: <https://github.com/wenjiudaijiugui/ScpTensor/actions/workflows/docs-ci.yml>
- `Codecov`: <https://codecov.io/gh/wenjiudaijiugui/ScpTensor>

Notes:

- Generate the badge image URL from the GitHub Actions page instead of hardcoding `badge.svg` endpoints here.
- This keeps the workflow README link-stable under strict link checks, especially when repository visibility or badge serving behavior changes.
